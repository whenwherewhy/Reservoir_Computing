...
class spike_encoding:
    ...
    def __init__(self, scheme, time_window=100, input_range=(0,255), output_freq_range=(0, 200)):
        
        self.scheme = scheme
        
        if scheme is 'rate_coding' or scheme is 'poisson_rate_coding':
    ...
    def encode(self, signal):
        ...
        if self.scheme is 'rate_coding':
            if len(signal.shape)>1:
                ...
                raise Exception('encoder() excepts signal with 2D. Reshape to (signal dimension x timestep) if signal is 1D')
        ...
        elif self.scheme is 'poisson_rate_coding':
            if len(signal.shape)>1:
                repeat = signal.shape[0] 
            else:
                raise Exception('encoder() excepts signal with 2D. Reshape to (signal dimension x timestep) if signal is 1D')
